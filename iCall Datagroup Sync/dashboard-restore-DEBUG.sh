#!/bin/bash
# Dashboard Datagroup Restore Script - DEBUG VERSION
...
build_records() {
    ...
    echo "DEBUG: Processing $record_type from $backup_file" >&2
    ...
    while IFS='|' read -r name data; do
        ...
        echo "DEBUG: Processing name='$name' data='$data'" >&2
        
        if [ "$record_type" = "pools" ]; then
            # Pools: name with sort order data
            if [ -n "$data" ] && [ "$data" != "" ]; then
                if [ "$first_record" = true ]; then
                    new_record="\"$name\" { data $data }"
                    first_record=false
                else
                    new_record=" \"$name\" { data $data }"
                fi
                records="$records$new_record"
                echo "DEBUG: Added pool record: $new_record" >&2
            else
                if [ "$first_record" = true ]; then
                    new_record="\"$name\" { data 999 }"
                    first_record=false
                else
                    new_record=" \"$name\" { data 999 }"
                fi
                records="$records$new_record"
                echo "DEBUG: Added default pool record: $new_record" >&2
            fi
        else
            ...
            if [ -n "$data" ] && [ "$data" != "" ] && [ "$data" != " " ]; then
                ...
                records="$records$new_record"
                echo "DEBUG: Added alias record with data: $new_record" >&2
            else
                if [ "$first_record" = true ]; then
                    new_record="\"$name\" { }"
                    first_record=false
                ...
                records="$records$new_record"
                echo "DEBUG: Added empty alias record: $new_record" >&2
            fi
        fi
    done < "$backup_file"
    
    echo "DEBUG: Final records string for $record_type:" >&2
    echo "DEBUG: '$records'" >&2
    echo "DEBUG: Length: ${#records}" >&2
...
perform_restore() {
    ...
    # Show the exact tmsh commands that will be executed
    echo ""
    echo "DEBUG: About to execute tmsh command for pools:"
    echo "tmsh modify ltm data-group internal \"$POOLS_DATAGROUP\" records replace-all-with { $pool_records }"
    echo ""
    echo "DEBUG: About to execute tmsh command for aliases:"
    echo "tmsh modify ltm data-group internal \"$ALIAS_DATAGROUP\" records replace-all-with { $alias_records }"
    echo ""
    ...
    if ! tmsh modify ltm data-group internal "$POOLS_DATAGROUP" records replace-all-with { $pool_records }; then
        ...
        echo "DEBUG: Failed command was: tmsh modify ltm data-group internal \"$POOLS_DATAGROUP\" records replace-all-with { $pool_records }"
        return 1
    fi
    ...
    if ! tmsh modify ltm data-group internal "$ALIAS_DATAGROUP" records replace-all-with { $alias_records }; then
        ...
        echo "DEBUG: Failed command was: tmsh modify ltm data-group internal \"$ALIAS_DATAGROUP\" records replace-all-with { $alias_records }"
        return 1
    fi
...
echo "=== Dashboard Datagroup Restore Tool - DEBUG VERSION ==="
echo ""
